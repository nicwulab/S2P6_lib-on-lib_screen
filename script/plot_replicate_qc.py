...
def plotting(df, cutoff, graph_suffix):
  df = df[df['totalfreq']>cutoff]
  ...
  pyo.plot(fig, filename='graph/QC/Expression_score_correlation'+graph_suffix+'.html')
  ...
  pyo.plot(fig, filename='graph/QC/Binding_score_correlation'+graph_suffix+'.html')

def main():
  df = pd.read_csv('result/mut_scores.tsv', sep='\t')
  plotting(df, 0, '')
  plotting(df, 0.00001, '_1*10-5')
  plotting(df, 0.00002, '_2*10-5')
